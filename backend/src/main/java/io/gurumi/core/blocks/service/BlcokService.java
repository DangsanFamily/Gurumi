...
public class BlockService {
    ...
    public String PostImages(BlockRequest blockRequest) {
        return blockRequest.getImageFiles().stream()
                .map(image -> s3Uploader.upload(image, "post"))
                .map(url -> createPostImage(block, url))
                .map(postImage -> postImage.getImageUrl())
                .collect(Collectors.toList());
    }

    public PostImage createPostImage(Block block, String url) {
